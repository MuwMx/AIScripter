...
from src.depth.og_dpt_utils.transform import Resize, NormalizeImage, PrepareForNet

from src.depth.distillanydepth.modeling.backbones.vit.ViT_DINO import (
    vit_large,
    vit_base,
)
from src.depth.distillanydepth.modeling.backbones.vit.ViT_DINO_reg import (
    vit_large_reg,
    vit_giant2_reg,
)
...
class DepthAnything(nn.Module, PyTorchModelHubMixin):
            ...
            if encoder == "vitl":
                if pretrain_type == "dinov2":
                    self.backbone = vit_large(
                        checkpoint=None, del_mask_token=del_mask_token
                    )
                else:
                    raise NotImplementedError
    ...
    def get_bins(self, bins_num):
        depth_bins_vec = torch.linspace(
            math.log(self.min_depth), math.log(self.max_depth), bins_num, device="cuda"
        )
    ...
    def register_depth_expectation_anchor(self, bins_num, B):
        ...
        self.register_buffer(
            "depth_expectation_anchor", depth_bins_vec, persistent=False
        )
