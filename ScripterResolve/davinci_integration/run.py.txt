...
# УЛЬТИМАТИВНЫЙ ФИКС ДЛЯ WINDOWS: Заставляем всю систему искать любые DLL в папке Резалва
RESOLVE_PATH = r"C:\Program Files\Blackmagic Design\DaVinci Resolve"
if os.path.exists(RESOLVE_PATH):
    # 1. Добавляем Резалв в PATH процесса, чтобы fusionscript смог запустить свои экзешники в фоне
    os.environ["PATH"] = RESOLVE_PATH + os.pathsep + os.environ.get("PATH", "")
    # 2. Перенаправляем поиск DLL для LoadLibrary
    ctypes.windll.kernel32.SetDllDirectoryW(RESOLVE_PATH)
    # 3. Дублируем официальные переменные среды прямо в память процесса
...
# Подключаем папки сторонних библиотек для внутреннего Питона Резалва
import glob
